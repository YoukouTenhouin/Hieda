...
- [ ] The documented canonical build and test commands succeed from a clean checkout.
- [ ] The packaged application launches as a native Qt Quick process and can create a Notebook at a user-selected path.
- [ ] The application can close and reopen a created Notebook without creating a second canonical content file.
- [ ] Opening a Notebook is mediated by the toolkit-neutral Notebook module; Qt and LMDB types do not cross its interface.
- [ ] Only one Notebook can be open at a time, and attempts to open unsupported or invalid inputs produce a visible error rather than a crash.
- [ ] Automated tests exercise create, close, and reopen behavior through the Notebook module using a temporary real Notebook file.
