...
- [x] Page Names accept the slash-separated segment grammar and limits, while invalid names fail
  without changing acknowledged state.
- [x] Exact hierarchy lookup distinguishes a materialized Named Page, a missing-prefix Page
  Preview required by a materialized descendant, and a name absent from the hierarchy.
- [x] Root and immediate-child enumeration returns deterministic bytewise-ASCII segment order in
...
- [x] The native hierarchy UI loads children lazily, keeps expansion state session-local, reveals
...
- [x] Activating a hierarchy Page Preview opens the same exact-name non-materialized destination
  used for unresolved links, creates no Block, and offers explicit creation.
- [x] Creating from a Page Preview materializes only that exact Named Page, preserves descendant
  placement, and turns the current destination into the Page in place.
- [x] Named Page creation is one Notebook-wide undoable action; undo and redo remove and restore
  the Page identity, timestamps, hierarchy effects, and exact logical state atomically.
- [x] Deleting a Named Page never deletes hierarchy descendants: its node becomes a Page Preview
...
- [x] Deleting a Named Page removes its complete contained Entry forest atomically; undo restores
  all identities, timestamps, Containment, order, and hierarchy state.
- [x] Creation, deletion, undo, redo, validation failures, persistence failures, revisions, and
  post-commit notifications obey the single-transaction contract.
- [x] Notebook-interface, adapter, persistence, reopen, failure-atomicity, and packaged UI smoke
