...
- Page Hierarchy derives exclusively from materialized Named Page names and required missing-prefix Page Previews. It provides exact lookup, bytewise ASCII sibling ordering, lazy revision-bound child enumeration, canonical non-materialized preview navigation, inclusive slash-boundary subtree predicates over Blocks, and atomic undoable creation, deletion, and rename cascades.
...
- Before implementation tickets are considered executable, the remaining decisions must settle the exact Notebook interface, authored-text grammar, journal editing behavior, persistence schema and migration protocol, data-safety contract, search semantics, Query language, Qt Quick interaction model, build/toolchain baseline, and acceptance gates.
