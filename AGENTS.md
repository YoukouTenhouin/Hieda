...
This workflow applies to implementation work. Design-only changes, including ADR and glossary
updates, do not require development and reflow branches unless they accompany an implementation.

For every new feature implementation, first synchronize local `master` with its upstream remote branch by fetching and fast-forwarding it. Then create a Git-safe, lowercase kebab-case `<feature-name>-dev` branch from the updated `master` and commit frequently in small, logical increments. After the feature is complete and the full test suite passes locally, push `<feature-name>-dev` and run its CI workflows. Make any required fixes on the dev branch, push them, and rerun CI until all workflows pass. Only then create `<feature-name>-reflow`. Reconstruct the work there as small, self-contained commits: each commit must include its relevant tests and fold later fixes into the commit that introduced the behavior, leaving the repository coherent at every step. Make all subsequent fixes on `<feature-name>-dev` first, then reconcile `<feature-name>-reflow` by folding each fix into the commit that introduced the affected behavior. The reflow history need not mirror development chronology, but its final tree must exactly match the dev branch. Verify this with `git diff --exit-code <feature-name>-dev <feature-name>-reflow`. After the reflow branch is ready, create a GitHub pull request from `<feature-name>-reflow` to `master`.
