...
auto
makeFileError(DWORD error) -> FileError
{
...
class ExclusiveFileLock::Impl final {
  public:
    ...
    Impl(HANDLE handle, HANDLE semaphore)
        : handle_(handle), semaphore_(semaphore)
    {
    }
    ...
    -> std::variant<ExclusiveFileLock, FileError>
{
    if (!create) {
        ...
        const auto semaphore =
            CreateSemaphoreW(nullptr, 1, 1, semaphoreName.c_str());
        ...
        return ExclusiveFileLock(
            std::make_unique<ExclusiveFileLock::Impl>(handle, semaphore));
    }
...
auto
syncParentDirectory(const std::filesystem::path&) -> std::optional<FileError>
{
    // MoveFileExW with MOVEFILE_WRITE_THROUGH does not return until publication
    // is complete.
    return std::nullopt;
}
