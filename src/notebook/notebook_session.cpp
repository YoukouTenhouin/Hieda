...
auto encodeBlock(const BlockRecord& block) -> std::vector<std::uint8_t> {
    ...
    if (block.pageKind) {
        const auto kind =
            static_cast<std::uint8_t>(static_cast<std::uint8_t>(*block.pageKind) + 1U);
    ...
    const auto validPage =
        ...
        ((block.pageKind == PageKind::journal && block.journalDate &&
         ...
         (block.pageKind == PageKind::named && !block.journalDate &&
          authored_text::validPageName(block.pageName) && validPageTitle(block.displayTitle))) &&
...
auto writeIncrementedRevision(MDB_txn* transaction, MDB_dbi metadata,
                              const std::filesystem::path& path) -> std::optional<NotebookError> {
    constexpr std::string_view keyText = "manifest";
    MDB_val key{keyText.size(), const_cast<char*>(keyText.data())};
    MDB_val value{};
...
auto rebuildSemanticReferenceIndexes(MDB_txn* transaction, const JournalDatabases& databases,
    ...
    while (result == MDB_SUCCESS && key.mv_size == lower.size() &&
        ...
        BlockId sourceId;
        std::memcpy(sourceId.bytes.data(), static_cast<const std::uint8_t*>(key.mv_data) + 1,
                    sourceId.bytes.size());
        ...
        const auto links = authored_text::pageLinks(source.value().authoredText);
        if (!links.empty()) {
            std::vector<std::uint8_t> encoded;
            appendU16(encoded, 1);
            appendU32(encoded, static_cast<std::uint32_t>(links.size()));
            for (const auto& link : links) {
                appendU32(encoded, static_cast<std::uint32_t>(link.sourceByteOffset));
                appendU32(encoded, static_cast<std::uint32_t>(link.sourceByteLength));
                appendU16(encoded, static_cast<std::uint16_t>(link.pageName.size()));
                encoded.insert(encoded.end(), link.pageName.begin(), link.pageName.end());

                MDB_val nameKey{link.pageName.size(), const_cast<char*>(link.pageName.data())};
                MDB_val pageValue{};
                const auto lookup =
                    mdb_get(transaction, databases.pagesByName, &nameKey, &pageValue);
                const auto resolved =
                    lookup == MDB_SUCCESS && pageValue.mv_size == sourceId.bytes.size();
                encoded.push_back(resolved ? 1U : 0U);
                if (resolved) {
                    const auto* target = static_cast<const std::uint8_t*>(pageValue.mv_data);
                    encoded.insert(encoded.end(), target, target + sourceId.bytes.size());
                    ...
                    return lookup == MDB_SUCCESS
                               ...
                               : errorFromLmdb(path, lookup, "resolve Page Link");
                }
            }
            auto sourceKey = blockKey(sourceId);
            MDB_val sourceValue{encoded.size(), encoded.data()};
            const auto put =
                mdb_put(transaction, databases.referencesBySource, &sourceKey, &sourceValue, 0);
            if (put != MDB_SUCCESS) {
                mdb_cursor_close(cursor);
    ...
    result = mdb_cursor_open(transaction, databases.referencesBySource, &sourceCursor);
    if (result != MDB_SUCCESS) {
        return errorFromLmdb(path, result, "open Page Link reverse indexing scan");
    }
    MDB_val sourceKey{};
    MDB_val sourceValue{};
    result = mdb_cursor_get(sourceCursor, &sourceKey, &sourceValue, MDB_FIRST);
    while (result == MDB_SUCCESS) {
        const auto* bytes = static_cast<const std::uint8_t*>(sourceValue.mv_data);
        if (sourceKey.mv_size != BlockId{}.bytes.size() || sourceValue.mv_size < 6 ||
            readU16(bytes) != 1) {
        ...
        for (std::uint32_t index = 0; index < count; ++index) {
            ...
            offset += 10;
            if (sourceValue.mv_size - offset < static_cast<std::size_t>(nameLength) + 1) {
                mdb_cursor_close(sourceCursor);
                return makeError(NotebookErrorCode::invalidNotebook, path,
            ...
            if (resolved) {
                ...
                reverseKey.insert(reverseKey.end(), target, target + BlockId{}.bytes.size());
                offset += nameLength + 1 + BlockId{}.bytes.size();
            } else {
                const auto name =
                    std::string_view{reinterpret_cast<const char*>(bytes + offset), nameLength};
                reverseKey = unresolvedPageLinkTargetPrefix(name);
                offset += nameLength + 1;
            }
            const auto* source = static_cast<const std::uint8_t*>(sourceKey.mv_data);
            reverseKey.insert(reverseKey.end(), source, source + sourceKey.mv_size);
            MDB_val reverseKeyValue{reverseKey.size(), reverseKey.data()};
            MDB_val empty{0, nullptr};
            const auto put =
                mdb_put(transaction, databases.referencesByTarget, &reverseKeyValue, &empty, 0);
            if (put != MDB_SUCCESS) {
                mdb_cursor_close(sourceCursor);
                return errorFromLmdb(path, put, "write Page Link reverse index");
            }
        }
        ...
        result = mdb_cursor_get(sourceCursor, &sourceKey, &sourceValue, MDB_NEXT);
    }
    mdb_cursor_close(sourceCursor);
    return result == MDB_NOTFOUND ? std::nullopt
...
class NotebookSession::Impl {
  public:
    ...
    auto eraseContainment(MDB_txn* transaction, const JournalDatabases& databases,
                          const OutlineEntryRecord& entry) const -> std::optional<NotebookError> {
        const auto path = info.value_or(NotebookInfo{}).path;
    ...
    static auto subtreeEnd(const std::vector<OutlineEntryRecord>& entries, std::size_t root)
        -> std::size_t {
    ...
    auto loadOutlineForDate(MDB_txn* transaction, const JournalDatabases& databases,
        ...
        if (!page || page.value().type != BlockType::page ||
            page.value().pageKind != PageKind::journal || page.value().journalDate != date) {
            return Result<std::optional<LoadedOutline>>::failure(
    ...
    void enforceHistoryBudget() {
        ...
        while (historyBytes > historyBudget && historyActionCount() > 1) {
            ...
            if (!crossPageUndo.empty() && crossPageUndo.front().sequence < oldestSequence) {
                oldestHistory = nullptr;
                oldestPageHistory = nullptr;
                oldestIsRedo = false;
            ...
            if (!crossPageRedo.empty() && crossPageRedo.back().sequence < oldestSequence) {
                oldestHistory = nullptr;
                oldestPageHistory = nullptr;
                oldestIsRedo = true;
                oldestIsCrossPage = true;
            }
            if (oldestHistory == nullptr && oldestPageHistory == nullptr && !oldestIsCrossPage) {
                break;
            }
    ...
    auto restoreOutline(MDB_txn* transaction, const JournalDatabases& databases, JournalDate date,
        ...
        for (const auto& entry : target->entries) {
            const BlockRecord block{
                BlockType::entry, entry.metadata, std::nullopt, entry.authoredText, {}, {},
                std::nullopt};
    ...
    auto applyHistory(JournalDate date, bool redo) -> Result<JournalPage> {
        ...
        return Result<JournalPage>::success(
            {date, target->page.metadata, publicJournalEntries(target->entries)});
    }
    ...
    auto readPageLinks(BlockId entryId) const -> Result<std::vector<PageLink>> {
        const auto path = info.value_or(NotebookInfo{}).path;
        MDB_txn* transaction = nullptr;
        auto result = mdb_txn_begin(environment, nullptr, MDB_RDONLY, &transaction);
        if (result != MDB_SUCCESS) {
        ...
        MDB_val encoded{};
        result = mdb_get(transaction, databases.value().referencesBySource, &sourceKey, &encoded);
        if (result == MDB_NOTFOUND) {
            mdb_txn_abort(transaction);
        ...
        if (result != MDB_SUCCESS || encoded.mv_size < 6) {
            ...
            return Result<std::vector<PageLink>>::failure(
                result == MDB_SUCCESS ? makeError(NotebookErrorCode::invalidNotebook, path,
                                      ...
                                      : errorFromLmdb(path, result, "read Page Link source index"));
        }
        const auto* bytes = static_cast<const std::uint8_t*>(encoded.mv_data);
        if (readU16(bytes) != 1) {
            mdb_txn_abort(transaction);
            return Result<std::vector<PageLink>>::failure(makeError(
                NotebookErrorCode::invalidNotebook, path, "unsupported Page Link index version"));
        }
        const auto count = readU32(bytes + 2);
        std::size_t offset = 6;
        ...
        for (std::uint32_t index = 0; index < count; ++index) {
            if (encoded.mv_size - offset < 11) {
                mdb_txn_abort(transaction);
                return Result<std::vector<PageLink>>::failure(makeError(
                    NotebookErrorCode::invalidNotebook, path, "Page Link occurrence is truncated"));
            }
            const auto sourceOffset = readU32(bytes + offset);
            const auto sourceLength = readU32(bytes + offset + 4);
            const auto nameLength = readU16(bytes + offset + 8);
            offset += 10;
            if (encoded.mv_size - offset < static_cast<std::size_t>(nameLength) + 1) {
                mdb_txn_abort(transaction);
                return Result<std::vector<PageLink>>::failure(makeError(
                    NotebookErrorCode::invalidNotebook, path, "Page Link name is truncated"));
            }
            std::string pageName(reinterpret_cast<const char*>(bytes + offset), nameLength);
            offset += nameLength;
            ...
            if (resolved) {
                if (encoded.mv_size - offset < BlockId{}.bytes.size()) {
                    mdb_txn_abort(transaction);
                    return Result<std::vector<PageLink>>::failure(makeError(
                        NotebookErrorCode::invalidNotebook, path, "Page Link target is truncated"));
                }
                BlockId targetId;
                std::memcpy(targetId.bytes.data(), bytes + offset, targetId.bytes.size());
                offset += targetId.bytes.size();
                auto block = readBlock(transaction, databases.value().blocks, targetId, path);
                ...
                target = PageSummary{block.value().metadata, block.value().pageName,
                                     block.value().displayTitle};
            }
            links.push_back({sourceOffset, sourceLength, std::move(pageName), std::move(target)});
        }
    ...
    auto readPagePreview(const std::string& name) const -> Result<PagePreview> {
        ...
        while (result == MDB_SUCCESS && key.mv_size == prefix.size() + BlockId{}.bytes.size() &&
            ...
            preview.sources.push_back(
                {source.value().metadata, source.value().authoredText, parentEntry});
            result = mdb_cursor_get(cursor, &key, &value, MDB_NEXT);
        }
        mdb_cursor_close(cursor);
        if (result != MDB_SUCCESS && result != MDB_NOTFOUND) {
    ...
    auto namedPageByName(MDB_txn* transaction, const JournalDatabases& databases,
                         std::string_view name) const -> Result<std::optional<PageSummary>> {
        const auto path = info.value_or(NotebookInfo{}).path;
        MDB_val key{name.size(), const_cast<char*>(name.data())};
        MDB_val value{};
        const auto result = mdb_get(transaction, databases.pagesByName, &key, &value);
        if (result == MDB_NOTFOUND) {
        ...
        if (value.mv_size != BlockId{}.bytes.size()) {
            return Result<std::optional<PageSummary>>::failure(makeError(
                NotebookErrorCode::invalidNotebook, path, "invalid Page name index value"));
        }
        BlockId id;
        std::memcpy(id.bytes.data(), value.mv_data, id.bytes.size());
        auto block = readBlock(transaction, databases.blocks, id, path);
        if (!block || block.value().type != BlockType::page ||
            block.value().pageKind != PageKind::named || block.value().pageName != name) {
            return Result<std::optional<PageSummary>>::failure(
                block ? makeError(NotebookErrorCode::invalidNotebook, path,
                                  "Page name index points to an invalid Named Page")
                      : block.error());
        }
        return Result<std::optional<PageSummary>>::success(PageSummary{
            block.value().metadata, block.value().pageName, block.value().displayTitle});
    }

    auto hierarchyNameHasChildren(MDB_txn* transaction, const JournalDatabases& databases,
                                  std::string_view name) const -> Result<bool> {
        const auto path = info.value_or(NotebookInfo{}).path;
        const auto prefix = std::string(name) + '/';
        MDB_cursor* cursor = nullptr;
        auto result = mdb_cursor_open(transaction, databases.pagesByName, &cursor);
        if (result != MDB_SUCCESS) {
            return Result<bool>::failure(
                errorFromLmdb(path, result, "open Page hierarchy child lookup"));
        }
        MDB_val key{prefix.size(), const_cast<char*>(prefix.data())};
        MDB_val value{};
        result = mdb_cursor_get(cursor, &key, &value, MDB_SET_RANGE);
        const auto hasChildren = result == MDB_SUCCESS && key.mv_size >= prefix.size() &&
                                 std::memcmp(key.mv_data, prefix.data(), prefix.size()) == 0;
        mdb_cursor_close(cursor);
        if (result != MDB_SUCCESS && result != MDB_NOTFOUND) {
            return Result<bool>::failure(
                errorFromLmdb(path, result, "read Page hierarchy child lookup"));
        }
        return Result<bool>::success(hasChildren);
    }

    auto readHierarchyNode(std::string name) const -> Result<std::optional<PageHierarchyNode>> {
        const auto path = info.value_or(NotebookInfo{}).path;
        MDB_txn* transaction = nullptr;
        auto result = mdb_txn_begin(environment, nullptr, MDB_RDONLY, &transaction);
        if (result != MDB_SUCCESS) {
        ...
        if (!databases) {
            mdb_txn_abort(transaction);
            return Result<std::optional<PageHierarchyNode>>::failure(databases.error());
        }
        auto page = namedPageByName(transaction, databases.value(), name);
        auto hasChildren = hierarchyNameHasChildren(transaction, databases.value(), name);
        if (!page || !hasChildren) {
            mdb_txn_abort(transaction);
            return Result<std::optional<PageHierarchyNode>>::failure(page ? hasChildren.error()
                                                                          : page.error());
        }
        if (!page.value() && !hasChildren.value()) {
            mdb_txn_abort(transaction);
            return Result<std::optional<PageHierarchyNode>>::success(std::nullopt);
        }
        const auto separator = name.rfind('/');
        auto localSegment = separator == std::string::npos ? name : name.substr(separator + 1);
        PageHierarchyNode node{std::move(name), std::move(localSegment), std::move(page).value(),
                               hasChildren.value()};
        mdb_txn_abort(transaction);
        return Result<std::optional<PageHierarchyNode>>::success(std::move(node));
    }

    auto readHierarchyChildren(const std::optional<std::string>& parentName,
                               const std::optional<std::string>& continuationCursor) const
        ...
        if (continuationCursor) {
            ...
            const auto revisionText = std::string_view(*continuationCursor).substr(0, firstBreak);
            const auto parsed = std::from_chars(
                revisionText.data(), revisionText.data() + revisionText.size(), cursorRevision);
            ...
            if (firstBreak == std::string::npos || secondBreak == std::string::npos ||
                ...
                return Result<PageHierarchyBatch>::failure(
                    makeError(NotebookErrorCode::staleHierarchyCursor, path,
                              "Page hierarchy cursor is stale; request a fresh first batch"));
            }
        ...
        if (result != MDB_SUCCESS) {
            return Result<PageHierarchyBatch>::failure(
                errorFromLmdb(path, result, "begin Page hierarchy enumeration"));
        }
        const auto fail = [&](NotebookError error) -> Result<PageHierarchyBatch> {
            mdb_txn_abort(transaction);
        ...
        MDB_cursor* cursor = nullptr;
        result = mdb_cursor_open(transaction, databases.value().pagesByName, &cursor);
        if (result != MDB_SUCCESS) {
            return fail(errorFromLmdb(path, result, "open Page hierarchy enumeration"));
        }
        ...
        MDB_val value{};
        result = scanStart.empty() ? mdb_cursor_get(cursor, &key, &value, MDB_FIRST)
                                   : mdb_cursor_get(cursor, &key, &value, MDB_SET_RANGE);
        PageHierarchyBatch batch;
        std::string previousSegment;
        while (result == MDB_SUCCESS) {
            const auto pageName =
                std::string_view(static_cast<const char*>(key.mv_data), key.mv_size);
            ...
            if (!segment.empty() && segment != previousSegment && segment > lastSegment) {
                ...
                if (!materialized || !hasChildren) {
                    mdb_cursor_close(cursor);
                    return fail(materialized ? hasChildren.error() : materialized.error());
                }
                batch.nodes.push_back(
                    {name, segment, std::move(materialized).value(), hasChildren.value()});
        ...
        mdb_cursor_close(cursor);
        if (result != MDB_SUCCESS && result != MDB_NOTFOUND) {
            return fail(errorFromLmdb(path, result, "read Page hierarchy enumeration"));
        }
        if (batch.nodes.size() == 101) {
    ...
    auto deletePage(BlockId pageId) -> Result<Page> {
        lastCommandCommitted = false;
    ...
    auto renamePage(BlockId pageId, std::string name, std::string displayTitle) -> Result<Page> {
        ...
        if (oldName != name) {
            ...
            result = mdb_cursor_open(transaction, databases.value().referencesByTarget, &cursor);
            if (result != MDB_SUCCESS) {
                return fail(errorFromLmdb(path, result, "open Page Link rewrite scan"));
            }
            ...
            while (result == MDB_SUCCESS && key.mv_size == prefix.size() + BlockId{}.bytes.size() &&
                ...
                for (const auto& occurrence : std::views::reverse(links)) {
                    if (occurrence.pageName == oldName) {
                        rewritten.replace(occurrence.sourceByteOffset + 2,
                                          occurrence.sourceByteLength - 4, name);
                    }
                }
                if (rewritten != source.value().authoredText) {
                    ...
                    textChanges.push_back(
                        {sourceId, source.value().authoredText, std::move(rewritten)});
                }
                result = mdb_cursor_get(cursor, &key, &value, MDB_NEXT);
            }
            mdb_cursor_close(cursor);
            if (result != MDB_SUCCESS && result != MDB_NOTFOUND) {
                return fail(errorFromLmdb(path, result, "scan Page Link rewrite sources"));
            }
            for (const auto& change : textChanges) {
                auto source =
                    readBlock(transaction, databases.value().blocks, change.sourceId, path);
                if (!source) {
                    return fail(source.error());
                }
                ...
                if (auto error =
                        writeBlock(transaction, databases.value().blocks, rewritten, path)) {
                    return fail(std::move(*error));
                }
            }
        }
    ...
    void recordPageHistory(std::optional<Page> before, std::optional<Page> after,
                           std::vector<MechanicalTextChange> mechanicalTextChanges = {}) {
        clearRedoHistory();
        if (!before && !after) {
        ...
        const auto estimatePageBytes = [](const std::optional<Page>& page) -> std::size_t {
            if (!page) {
        ...
        PageHistoryAction action{nextHistorySequence++, std::move(before), std::move(after),
                                 std::move(mechanicalTextChanges), 0};
        action.estimatedBytes = sizeof(PageHistoryAction) + estimatePageBytes(action.before) +
                                estimatePageBytes(action.after);
        for (const auto& change : action.mechanicalTextChanges) {
            action.estimatedBytes +=
                sizeof(MechanicalTextChange) + change.before.size() + change.after.size();
        }
        historyBytes += action.estimatedBytes;
        history.undo.push_back(std::move(action));
        enforceHistoryBudget();
    ...
    auto restorePageState(BlockId pageId, const std::optional<Page>& target,
                          const std::vector<MechanicalTextChange>& textChanges = {},
                          bool restoreAfterText = false) -> Result<std::optional<Page>> {
        lastCommandCommitted = false;
        const auto path = info.value_or(NotebookInfo{}).path;
        MDB_txn* transaction = nullptr;
        auto result = mdb_txn_begin(environment, nullptr, 0, &transaction);
        ...
        const auto fail = [&](NotebookError error) -> Result<std::optional<Page>> {
            mdb_txn_abort(transaction);
        ...
        auto currentBlock = readBlock(transaction, databases.value().blocks, pageId, path);
        if (currentBlock) {
            ...
            auto current = loadOutline(transaction, databases.value(), currentBlock.value());
            if (!current) {
            ...
            for (const auto& entry : current.value().entries) {
                ...
                if (result != MDB_SUCCESS) {
                    return fail(errorFromLmdb(path, result, "remove Page Entry for history"));
                }
            ...
            MDB_val nameKey{currentBlock.value().pageName.size(),
                            const_cast<char*>(currentBlock.value().pageName.data())};
            result = mdb_del(transaction, databases.value().pagesByName, &nameKey, nullptr);
            if (result != MDB_SUCCESS) {
                return fail(errorFromLmdb(path, result, "remove Page name for history"));
            }
            auto key = blockKey(pageId);
            result = mdb_del(transaction, databases.value().blocks, &key, nullptr);
            if (result != MDB_SUCCESS) {
                return fail(errorFromLmdb(path, result, "remove Page for history"));
            }
            if (auto error = removeTypeIndex(transaction, databases.value().blocksByType,
                                             BlockType::page, pageId)) {
                return fail(std::move(*error));
            }
        } else if (currentBlock.error().code != NotebookErrorCode::blockNotFound) {
            return fail(currentBlock.error());
        }
        const auto restoreMechanicalText = [&]() -> std::optional<NotebookError> {
            for (const auto& change : textChanges) {
                auto source =
                    readBlock(transaction, databases.value().blocks, change.sourceId, path);
                if (!source) {
                ...
                block.authoredText = restoreAfterText ? change.after : change.before;
                if (auto error = writeBlock(transaction, databases.value().blocks, block, path)) {
                    return error;
                }
            }
        ...
        BlockRecord restoredBlock{BlockType::page, target->metadata,     std::nullopt,   {},
        ...
        MDB_val nameKey{restoredBlock.pageName.size(), restoredBlock.pageName.data()};
        MDB_val pageValue{pageId.bytes.size(), pageId.bytes.data()};
        result = mdb_put(transaction, databases.value().pagesByName, &nameKey, &pageValue,
                         MDB_NOOVERWRITE);
        if (result != MDB_SUCCESS) {
            return fail(result == MDB_KEYEXIST
                            ? makeError(NotebookErrorCode::pageNameConflict, path,
                                        "Page name is already in use")
                            : errorFromLmdb(path, result, "restore Page name for history"));
        }
        ...
        incrementCachedRevision();
        lastCommandCommitted = true;
        return Result<std::optional<Page>>::success(publicPage(std::move(restored)));
    }

    auto applyPageHistory(BlockId pageId, bool redo) -> Result<std::optional<Page>> {
        auto& history = pageHistory(pageId);
        auto& source = redo ? history.redo : history.undo;
        if (source.empty()) {
            return Result<std::optional<Page>>::failure(makeError(
                redo ? NotebookErrorCode::redoUnavailable : NotebookErrorCode::undoUnavailable,
                info.value_or(NotebookInfo{}).path, "no Page edit is available"));
        }
        const auto target = redo ? source.back().after : source.back().before;
        auto result = restorePageState(pageId, target, source.back().mechanicalTextChanges, redo);
        if (!result) {
            return result;
        }
    ...
    auto updateEntry(BlockId entryId, std::string authoredText) -> Result<OutlineEntryRecord> {
        lastCommandCommitted = false;
        const auto path = info.value_or(NotebookInfo{}).path;
        MDB_txn* transaction = nullptr;
        auto result = mdb_txn_begin(environment, nullptr, 0, &transaction);
        ...
        const auto fail = [&](NotebookError error) -> Result<OutlineEntryRecord> {
            mdb_txn_abort(transaction);
    ...
    auto insertNestedEntry(JournalDate date, std::optional<BlockId> afterEntry,
        ...
        recordHistory(date, pageWasCreated ? std::nullopt : std::optional<LoadedOutline>{before},
                      outline);
        return Result<JournalPage>::success(
            {date, outline.page.metadata, publicJournalEntries(outline.entries)});
    }
    ...
    auto deleteSubtrees(const std::vector<BlockId>& entryIds) -> Result<JournalPage> {
        ...
        const auto hasAncestorIn = [&](const OutlineEntryRecord& entry,
    ...
    auto moveEntryToPage(BlockId entryId, const PageAddress& destinationAddress,
                         std::optional<BlockId> afterEntry) -> Result<std::vector<LoadedOutline>> {
        lastCommandCommitted = false;
        const auto path = info.value_or(NotebookInfo{}).path;
        MDB_txn* transaction = nullptr;
        auto result = mdb_txn_begin(environment, nullptr, 0, &transaction);
        ...
        const auto fail = [&](NotebookError error) -> Result<std::vector<LoadedOutline>> {
            mdb_txn_abort(transaction);
            return Result<std::vector<LoadedOutline>>::failure(std::move(error));
        };
        auto databases = openJournalDatabases(transaction, path);
        if (!databases) {
            return fail(databases.error());
        }
        auto sourceResult = loadOutlineForEntry(transaction, databases.value(), entryId);
        if (!sourceResult) {
        ...
        if (const auto* pageId = std::get_if<BlockId>(&destinationAddress)) {
            auto block = readBlock(transaction, databases.value().blocks, *pageId, path);
            if (!block || block.value().type != BlockType::page ||
                block.value().pageKind != PageKind::named) {
                return fail(block ? makeError(NotebookErrorCode::pageNotFound, path,
                                              "move destination is not a Named Page")
                                  : block.error());
            }
            auto loaded = loadOutline(transaction, databases.value(), std::move(block).value());
            if (!loaded) {
                return fail(loaded.error());
            }
            ...
            auto loaded = loadOutlineForDate(transaction, databases.value(), date);
            if (!loaded) {
                return fail(loaded.error());
            }
            ...
            if (loadedPage) {
                ...
                MDB_val pageValue{page.metadata.id.bytes.size(), page.metadata.id.bytes.data()};
                result = mdb_put(transaction, databases.value().journalByDate, &dateIndexKey,
                                 &pageValue, MDB_NOOVERWRITE);
                if (result != MDB_SUCCESS) {
                    return fail(errorFromLmdb(path, result, "materialize move destination"));
                }
        ...
        const auto found = std::ranges::find_if(source.entries, [&](const auto& entry) -> bool {
            return entry.metadata.id == entryId;
        });
        ...
        const auto sourceIndex =
            static_cast<std::size_t>(std::distance(source.entries.begin(), found));
        ...
        source.entries.erase(source.entries.begin() + static_cast<std::ptrdiff_t>(sourceIndex),
                             source.entries.begin() + static_cast<std::ptrdiff_t>(sourceEnd));
        ...
        if (afterEntry) {
            const auto insertion =
                std::ranges::find_if(destination.entries, [&](const auto& entry) -> bool {
                    return entry.metadata.id == *afterEntry;
                });
            ...
            const auto index =
                static_cast<std::size_t>(std::distance(destination.entries.begin(), insertion));
        ...
        destination.entries.insert(destination.entries.begin() +
                                       static_cast<std::ptrdiff_t>(destinationIndex),
                                   moving.begin(), moving.end());

        auto movedBlock = readBlock(transaction, databases.value().blocks, entryId, path);
        if (!movedBlock) {
        ...
        if (auto error =
                writeBlock(transaction, databases.value().blocks, movedBlockRecord, path)) {
            return fail(std::move(*error));
        }
        if (auto error = touchContainer(transaction, databases.value(), source, oldParent, now)) {
            return fail(std::move(*error));
        }
        if (auto error =
                touchContainer(transaction, databases.value(), destination, newParent, now)) {
            return fail(std::move(*error));
        }
        if (auto error = rewriteContainment(transaction, databases.value(), beforeSource, source)) {
            return fail(std::move(*error));
        }
        const LoadedOutline emptyDestination{destination.page, {}};
        if (auto error =
                rewriteContainment(transaction, databases.value(),
        ...
        lastCommandCommitted = true;
        clearRedoHistory();
        CrossPageHistoryAction action{
            nextHistorySequence++, beforeSource, beforeDestination, source, destination, 0};
    ...
    auto applyCrossPageHistory(bool redo) -> Result<std::vector<LoadedOutline>> {
        lastCommandCommitted = false;
        auto& sourceActions = redo ? crossPageRedo : crossPageUndo;
        if (sourceActions.empty()) {
            return Result<std::vector<LoadedOutline>>::failure(makeError(
                redo ? NotebookErrorCode::redoUnavailable : NotebookErrorCode::undoUnavailable,
                info.value_or(NotebookInfo{}).path, "no cross-Page edit is available"));
        }
        const auto& action = sourceActions.back();
        const auto& currentSource = redo ? action.beforeSource : action.afterSource;
        const auto& currentDestination =
            redo ? action.beforeDestination : std::optional<LoadedOutline>{action.afterDestination};
        const auto& targetSource = redo ? action.afterSource : action.beforeSource;
        const auto& targetDestination =
            redo ? std::optional<LoadedOutline>{action.afterDestination} : action.beforeDestination;
        const auto path = info.value_or(NotebookInfo{}).path;
        MDB_txn* transaction = nullptr;
        auto result = mdb_txn_begin(environment, nullptr, 0, &transaction);
        if (result != MDB_SUCCESS) {
        ...
        if (auto error =
                writeBlock(transaction, databases.value().blocks, targetSource.page, path)) {
            return fail(std::move(*error));
        }
        if (targetDestination) {
            ...
            if (!currentDestination) {
                ...
                if (targetDestination->page.journalDate) {
                    const auto encodedDate = dateKey(*targetDestination->page.journalDate);
                    MDB_val dateIndexKey{encodedDate.size(),
                                         const_cast<std::uint8_t*>(encodedDate.data())};
                    MDB_val pageValue{
                        targetDestination->page.metadata.id.bytes.size(),
                        const_cast<std::byte*>(targetDestination->page.metadata.id.bytes.data())};
                    result = mdb_put(transaction, databases.value().journalByDate, &dateIndexKey,
                                     &pageValue, MDB_NOOVERWRITE);
                    if (result != MDB_SUCCESS) {
                        return fail(
                            errorFromLmdb(path, result, "restore Journal move destination"));
                    }
                }
            }
        } else if (currentDestination) {
            auto pageKey = blockKey(currentDestination->page.metadata.id);
            result = mdb_del(transaction, databases.value().blocks, &pageKey, nullptr);
            if (result != MDB_SUCCESS) {
                return fail(errorFromLmdb(path, result, "remove virtualized Journal Page"));
            }
            if (auto error = removeTypeIndex(transaction, databases.value().blocksByType,
            ...
            if (currentDestination->page.journalDate) {
                const auto encodedDate = dateKey(*currentDestination->page.journalDate);
                MDB_val dateIndexKey{encodedDate.size(),
                                     const_cast<std::uint8_t*>(encodedDate.data())};
                result =
                    mdb_del(transaction, databases.value().journalByDate, &dateIndexKey, nullptr);
                if (result != MDB_SUCCESS) {
                    return fail(errorFromLmdb(path, result, "virtualize Journal move destination"));
                }
            }
        }
        const auto writeEntries =
            ...
            for (const auto& entry : outline.entries) {
                auto block =
                    readBlock(transaction, databases.value().blocks, entry.metadata.id, path);
                if (!block) {
                    return block.error();
                }
                ...
                if (auto error =
                        writeBlock(transaction, databases.value().blocks, restored, path)) {
                    return error;
                }
            }
            const LoadedOutline empty{outline.page, {}};
            return rewriteContainment(transaction, databases.value(), empty, outline);
        };
...
#ifdef HIEDA_TESTING
void NotebookSessionTestAccess::rejectNextCommit(NotebookSession& session) {
...
auto NotebookSession::pageHierarchyNode(std::string name) const
    ...
    if (!authored_text::validPageName(name)) {
        return Result<std::optional<PageHierarchyNode>>::failure(
            makeError(NotebookErrorCode::invalidPageName, impl_->info->path, "invalid Page name"));
    }
    return impl_->readHierarchyNode(std::move(name));
}

auto NotebookSession::pageHierarchyChildren(std::optional<std::string> parentName,
    ...
    if (parentName && !authored_text::validPageName(*parentName)) {
        return Result<PageHierarchyBatch>::failure(makeError(
            NotebookErrorCode::invalidPageName, impl_->info->path, "invalid parent Page name"));
    }
...
auto NotebookSession::deletePage(BlockId pageId) -> Result<Page> {
    std::vector<std::function<void()>> callbacks;
    auto result = [&]() -> Result<Page> {
        std::scoped_lock lock(impl_->mutex);
        if (!impl_->info) {
...
auto NotebookSession::pageLinks(BlockId entryId) const -> Result<std::vector<PageLink>> {
    std::scoped_lock lock(impl_->mutex);
    if (!impl_->info) {
        return Result<std::vector<PageLink>>::failure(
            makeError(NotebookErrorCode::notebookNotOpen, {}, "a Notebook must be open"));
...
auto NotebookSession::followPageLink(BlockId entryId, std::size_t sourceByteOffset) const
    ...
    const auto found = std::ranges::find_if(links.value(), [&](const auto& link) -> bool {
        return sourceByteOffset >= link.sourceByteOffset &&
               sourceByteOffset < link.sourceByteOffset + link.sourceByteLength;
    });
...
auto NotebookSession::pagePreview(std::string name) const -> Result<PagePreview> {
    std::scoped_lock lock(impl_->mutex);
    if (!impl_->info) {
        return Result<PagePreview>::failure(
            makeError(NotebookErrorCode::notebookNotOpen, {}, "a Notebook must be open"));
...
auto unifiedPage(const Page& page) -> OutlinePage {
    ...
    for (const auto& entry : page.entries) {
        result.entries.push_back({entry.metadata, entry.authoredText, entry.parentEntry});
    }
    return result;
}

auto unifiedPage(const JournalPage& page) -> OutlinePage {
...
auto unifiedPage(const LoadedOutline& outline) -> OutlinePage {
    OutlinePage result;
    if (outline.page.pageKind == PageKind::journal) {
        result.kind = PageKind::journal;
        result.journalDate = outline.page.journalDate;
...
auto NotebookSession::outline(PageAddress address) const -> Result<OutlinePage> {
...
auto NotebookSession::insertEntry(PageAddress address, std::optional<BlockId> afterEntry,
                                  std::string authoredText) -> Result<OutlinePage> {
    if (const auto* pageId = std::get_if<BlockId>(&address)) {
        auto result = insertPageEntry(*pageId, afterEntry, std::move(authoredText));
        return result ? Result<OutlinePage>::success(unifiedPage(result.value()))
                      : Result<OutlinePage>::failure(result.error());
    }
    auto result =
        insertJournalEntry(std::get<JournalDate>(address), afterEntry, std::move(authoredText));
    return result ? Result<OutlinePage>::success(unifiedPage(result.value()))
                  : Result<OutlinePage>::failure(result.error());
}
...
auto NotebookSession::moveEntryToPage(BlockId entryId, PageAddress destination,
    ...
    auto result = [&]() -> Result<std::vector<OutlinePage>> {
        ...
        if (const auto* date = std::get_if<JournalDate>(&destination);
            ...
            return Result<std::vector<OutlinePage>>::failure(makeError(
                NotebookErrorCode::invalidJournalDate, impl_->info->path, "invalid Journal date"));
        }
...
auto NotebookSession::editCapabilities() const -> Result<EditCapabilities> {
    std::scoped_lock lock(impl_->mutex);
    if (!impl_->info) {
...
auto NotebookSession::undoEdit() -> Result<std::vector<OutlinePage>> {
    std::vector<std::function<void()>> callbacks;
    auto result = [&]() -> Result<std::vector<OutlinePage>> {
        std::scoped_lock lock(impl_->mutex);
        if (!impl_->info) {
        ...
        BlockId pageId{};
        for (const auto& history : impl_->journalHistory) {
            if (!history.undo.empty() && history.undo.back().sequence > sequence) {
                kind = Kind::journal;
        ...
        if (!impl_->crossPageUndo.empty() && impl_->crossPageUndo.back().sequence > sequence) {
            kind = Kind::cross;
        }
        std::vector<OutlinePage> pages;
...
auto NotebookSession::redoEdit() -> Result<std::vector<OutlinePage>> {
    std::vector<std::function<void()>> callbacks;
    auto result = [&]() -> Result<std::vector<OutlinePage>> {
        std::scoped_lock lock(impl_->mutex);
        if (!impl_->info) {
        ...
        BlockId pageId{};
        for (const auto& history : impl_->journalHistory) {
            if (!history.redo.empty() && history.redo.back().sequence < sequence) {
                kind = Kind::journal;
        ...
        if (!impl_->crossPageRedo.empty() && impl_->crossPageRedo.back().sequence < sequence) {
            kind = Kind::cross;
        }
        std::vector<OutlinePage> pages;
        if (kind == Kind::journal) {
