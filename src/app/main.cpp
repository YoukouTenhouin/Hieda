...
auto main(int argc, char* argv[]) -> int {
    ...
    if (QCoreApplication::arguments().contains(QStringLiteral("--smoke-test"))) {
        ...
        const auto path = smokeDirectory->filePath(QStringLiteral("hierarchy-smoke.hieda"));
        controller.createNotebook(QUrl::fromLocalFile(path));
        const auto created =
            controller.createPage(QStringLiteral("work/client/alpha"), QStringLiteral("Alpha"));
        controller.navigateToPageName(QStringLiteral("work/client"));
        const auto openedPreview = controller.currentPagePreview();
        const auto materialized = controller.createCurrentPage(QStringLiteral("Client"));
        const auto deleted = controller.deleteCurrentPage();
        const auto undone =
            controller.undoOutlineEdit().value(QStringLiteral("succeeded")).toBool();
        const auto restored = !controller.currentPagePreview();
        const auto redone =
            controller.redoOutlineEdit().value(QStringLiteral("succeeded")).toBool();
        const auto succeeded = smokeDirectory->isValid() && created && openedPreview &&
                               materialized && deleted && undone && restored && redone &&
                               controller.currentPagePreview() &&
                               controller.pageHierarchy()->rowCount() == 1;
        QTimer::singleShot(0, &application, [&application, succeeded]() -> void {
